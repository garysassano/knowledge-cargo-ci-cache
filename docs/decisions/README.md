...
- The pragmatic RunsOn default combines mise-managed tool setup, Magic Cache, input-only `rust-cache`, and a clean target. It avoids whole-target growth while retaining reusable tool and dependency downloads. Input-only caching does not reuse compiler output and should be removed when its setup cost is effectively tied with normal downloads. See [the clean-target approach](../approaches/clean-target.md).
