...
| [`mr-boxington` vs `sccache`](mr-boxington-vs-sccache.md)                            | Separates same-job, historical path-mapping, corrected mbx 1.11.1, action-transport, native-S3, and mbx 1.15.0 qualification trials.                                   |
| [Mr. Boxington 1.15 target-mode features](mr-boxington-target-features.md)            | Records target adoption, detached collection, statistics latency, incremental-state transport, and enabled sharing behavior without a cross-tool comparison.           |
