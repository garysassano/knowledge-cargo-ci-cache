...
- Re-check the [sticky Git modes](../reference/runson-cache-and-disk-details.md#sticky-git-checkout-mode): mirror scope, the rule that they run before checkout, the separate invocation for workspace-relative records, `token` handling, and whether Cargo Git fetches go through the global `insteadOf` rewrite.
