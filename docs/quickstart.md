...
This shape is low-risk because it keeps tool setup and dependency downloads reusable without persisting mutable target state. Prefer no Rust cache when input-only setup is effectively tied with normal dependency downloads. For frequently changing PR workloads, canary S3-backed `sccache` and Mr. Boxington object mode with mbx 1.15.0, action 1.4.0, and explicit `backend: github` and `github-cache-mode: objects`. In the controlled September 22 retest, mbx 1.15.0 was effectively tied with 1.12.0 on three exact-warm workload samples, restored seven additional actions, and eliminated six native-library bypasses; its workload median was 80.83 seconds versus 93.93 seconds for sccache. Add a separate Cargo-input archive only when dependency-download timing justifies it. Mr. Boxington target mode was faster in an earlier immediate exact-warm pair, but its current package-name pruning is not size-bounded or generation-aware; it remains a narrow stable-workload option until changed-source soak testing rules out the target growth seen in production. The canonical decision record is [Decisions](decisions/README.md).
