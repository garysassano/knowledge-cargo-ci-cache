...
| Sticky `git` mode                        | Bare mirror of workflow-repository history                                                                  | Loopback proxy serves checkout from disk; restore and commit EBS-backed snapshots | Clean                    | Disk lineage                              | Last completed clean snapshot in the lineage   |
...
## Sticky Git checkout mode

Source refresh: 2026-09-26, pinned to `runs-on/action` v2.3.1. The modes first shipped in [v2.3.0](https://github.com/runs-on/action/releases/tag/v2.3.0). This summary comes from the [released README](https://github.com/runs-on/action/blob/v2.3.1/README.md#git-mode-fast-checkouts), the [mode definitions](https://github.com/runs-on/action/blob/v2.3.1/internal/stickydisk/modes.go), the [invocation validation](https://github.com/runs-on/action/blob/v2.3.1/internal/stickydisk/stickydisk.go), and the [proxy source](https://github.com/runs-on/action/tree/v2.3.1/internal/gitproxy). It has not been deployed or measured.

| Aspect                               | `git` (alias `checkout`)                                                                                                                       | `git-full`                                           |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Mirror scope                         | Workflow-repository history reachable from `GITHUB_SHA`, plus branch or tag tips that the workflow repository requests, in a private namespace | Every requested github.com repository, with all refs |
| Other repositories and unlisted SHAs | Forwarded to github.com unchanged                                                                                                              | Mirrored                                             |
| Cold cost                            | Bounded by the workflow repository's history                                                                                                   | Can need a much larger download and more disk space  |

Behavior shared by both modes:

- Setup starts a loopback smart-HTTP proxy and adds a global `url.<proxy>/github.com/.insteadOf https://github.com/` rewrite. It also appends an identity `url.https://github.com/.pushInsteadOf`, so pushes go straight to GitHub with the pusher's own credential.
- Mirror syncs and forwarded reads use the `token` input, which defaults to `github.token`. Other private repositories need a token with access to them, because the rewritten URL no longer matches the credentials that `actions/checkout` set up.
- LFS, receive-pack, objects missing from the mirror, and mirror failures are forwarded to github.com. A mirroring failure should fall back to github.com instead of failing the job.
- Constraints: the action runs before `actions/checkout`, one invocation cannot combine a Git mode with workspace-relative records, and `git` and `git-full` are mutually exclusive. The modes are Linux-only. They do not rewrite SSH remotes and do not speed up container jobs or GitHub Enterprise Server.
- The post step stops the proxy and cleans interrupted repacks before the snapshot. A failed proxy stop fails the job instead of risking an inconsistent snapshot.

Cargo interaction, which is inference and has not been tested: the rewrite is global Git configuration, so any Git client in the job that honors `insteadOf` also sends its github.com fetches through the proxy. Cargo with `net.git-fetch-with-cli = true` is one example. In `git` mode, Cargo Git dependencies count as other repositories and are forwarded upstream. In `git-full` mode, they may also be mirrored on the disk, duplicating what the `rust` mode keeps under `~/.cargo/git`. A private Git dependency that relied on credentials for `https://github.com/` depends on the `token` input having access. It has not been checked whether Cargo's default libgit2 or gitoxide transport applies the global rewrite. Test with a public and a private Git dependency before adopting either mode.

Trust notes:

- In the pinned proxy source, forwarded reads replace the job's opaque client credential with the `token` identity. A PAT passed as `token` therefore gives read access to any process in the job that uses the configured Git remote. Prefer the default token. If a PAT is needed, limit it to read access on the repositories involved.
- Mirrors persist in the disk lineage, which pull-request jobs may restore through [default-branch fallback](#sticky-disk-lineage-and-fallback). With `git-full` and a cross-repository PAT, objects from other private repositories persist there too. Do not use that combination on lineages that less-trusted jobs restore.
...
| Sticky Git modes                | Checked September 26, 2026. The website lists only `git`/`checkout`, described as GitHub.com repository mirrors. In v2.3.1, `git` covers only workflow-repository history, and `git-full` provides full mirrors. Use the released README and the [Git-mode details](#sticky-git-checkout-mode).                                               |
